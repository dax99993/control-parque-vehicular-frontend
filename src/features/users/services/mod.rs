...
use crate::services::request::{request_get, request_delete, request_patch, request_multipart_patch, request_image};
use crate::error::Error;

// General routes
pub async fn request_imagen_usuario(file: String) -> Result<Vec<u8>, Error> {
    request_image(format!("/api/users/picture/{file}")).await
}
